  ...
  make_sat_data <- function(start_date = "2019-12-01"
    ...
    if(sum(grepl("sf", class(settings$boundary)))) {
      ...
      bbox_use_epsg <- sf::st_bbox(settings$boundary
                                   , crs = 4326
                                   ) %>%
        sf::st_as_sfc() %>%
        sf::st_transform(crs = settings$use_epsg) %>%
    ...
    use_extent <- list(left = round(floor(bbox_use_epsg["xmin"][[1]]), -2) - bbox_adj
    ...
    purrr::walk(head(get_bands, -1)
                , function(x) {
                  ...
                  out_file <- fs::path(settings$ras_save_dir
      ...
      purrr::iwalk(categorical[-1]
                   , ~ {
                     ...
                     if(run) {
                       ...
                       temp_file <- fs::path(settings$ras_save_dir
                                             , "temp"
                                             , paste0(.y, "__", start_date, ".tif")
                                             )
    ...
    if(return_stack) {
      ...
      stack <- fs::dir_info(settings$ras_save_dir
                            , regexp = "tif$"
                            ) %>%
