...
The `envRas` project makes use of the `envRaster` package `r cite_package("envRaster")` to abstract all these steps into the single function `get_sat_data`. 
...
A regular cube is then defined via the `gdalcubes` function `cube_view`. Here an extent is required: a bounding box with the addition of bounding dates. In this case the coordinate system for the extent should be in the desired coordinate system for the resulting cube ([epsg: `r settings$epsg_proj`](https://epsg.io/7845)). The `make_sat_cube` takes the spatial details from the base_grid supplied and the temporal details from the period supplied.
